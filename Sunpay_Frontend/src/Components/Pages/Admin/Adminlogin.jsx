...
export default function Adminlogin({ setAdmLoggedIn }, { AdmLoggedIn }) {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://new.sunpay.co.in/api/login/', {
        method: 'POST',
        headers: {
    ...
    try {
      ...
      if (result.message === 'Login Successful') {
        const apidata = result.data;
          setisauth(true)
          setAdmLoggedIn(true);
          if(apidata.role_id===3 || apidata.role_id===4){
  ...
  return (
    <div className='adminlogin'>
      ...
      <div className="md:w-1/2 border-4 border-red-900 bg-white p-16 rounded-xl max-w-sm shadow-lg shadow-white">
       ...
       {isVisible && (
      <div className="md:w-1/2 border-4 border-red-900 bg-white p-16 rounded-xl max-w-sm shadow-lg shadow-white">
        <p className='text-4xl text-center mb-8 font-bold'>Enter TPIN</p>
        ...
        <div className="text-center md:text-center md:mt-4">
          <button onClick={handletpin} className="mt-4 bg-green-700 hover:bg-green-700 px-6 py-3 text-white uppercase rounded text-xl tracking-wider" type="submit">Login</button>
        </div>

        <div className="text-center md:text-center md:mt-4">
          <button onClick={handleback} className="mt-4 bg-red-600 hover:bg-blue-700 px-6 py-3 text-white uppercase rounded text-xl tracking-wider" type="submit">Back</button>
        </div>
      </div>
       )}
    </section>
