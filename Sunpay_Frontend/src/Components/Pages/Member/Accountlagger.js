...
const Accountlagger = (props) => {
  ...
  const showalltrans = async () => {
    try {
      const transresp = await axios.get(`http://118.139.167.172/api/wallettransaction/${props.data.id}/`);
                    ...
                    <tbody>
                      {allwalltrans.map((item, index) => (
                        <tr key={item.id} className='bg-white'>
                          ...
                          <td className={`px-6 py-3 border border-black whitespace-nowrap ${item.transaction_direction_display === 'CREDIT' ? 'text-green-600' : item.transaction_direction_display === 'DEBIT' ? 'text-red-600' : 'text-black-600'}`}>₹{item.amount}</td>
