...
const DMTreport = (props) => {
  ...
  const getdmttrans = async() => {
    try {
      ...
      const response = await axios.get(`http://118.139.167.172/api/dmttransaction/${currentuser.id}/`);
                 ...
                 <table className=" text-sm text-left rtl:text-right mt-2 border border-black text-gray-500 dark:text-gray-400 border-collapse ">
                    ...
                    {alldmttrans.map((item, index) => (
                        <tr key={item.id} className='bg-white'>
                          ...
                          <td scope="col" className="px-6 py-3 border border-black">{item.bene_name}</td>
                          <td scope="col" className="px-6 py-3 border border-black">{item.amount}</td>
                          <td scope="col" className="px-6 py-3 border border-black">{item.charge}</td>
