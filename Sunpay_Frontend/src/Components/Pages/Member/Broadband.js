  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredOptions = data.filter(option => option.type === 9);
    ...
    <div>
      <div className='Rechargehistory p-4'>
        <div className='bg-slate-400 w-full p-2 border-2 border-red-200 px-6'>
          <h1 className=' font-bold text-2xl border-b-2 text-white border-gray-400'>Broadband Bill Pay</h1>
          ...
          {!show && (
            <div>
              <div className='mt-4 flex gap-8 flex-wrap'>
                ...
                <div>
                  <select onChange={(e) => setCompany(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
              ...
              <div className='mt-4 flex gap-4'>
                <label className="mt-2 text-white w-52 text-2xl">Customer ID</label><br/>
                <input className="text-lg px-4 border border-solid border-gray-300 rounded" onChange={(e) => setNumber(e.target.value)} type="text" placeholder="Enter Customer ID" />
              </div>

              <div className='mt-4 flex gap-4'>
                <button onClick={handleClick} className="bg-blue-600 hover:bg-blue-700 px-6 py-3 text-white uppercase rounded text-sm tracking-wider" type="button">Confirm</button>
              </div>
            </div>
          )}
          ...
          {show && (
            <div>
              ...
              <div className='mt-4 flex gap-4'>
                <button onClick={handleSubmit} className="bg-blue-600 hover:bg-blue-700 px-6 py-3 text-white uppercase rounded text-sm tracking-wider" type="button">Submit</button>
