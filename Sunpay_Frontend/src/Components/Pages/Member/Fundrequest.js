...
const Fundrequest = (props) => {  
  ...
  const fetchBank = async () => {
    try {
      ...
      const response = await axios.get('http://new.sunpay.co.in/api/companybanks/'); // Assuming '/api/options' is your backend endpoint
  ...
  const Timestamp = today
    ...
    const submitfundrequest = async(e) => {
      ...
      const response = await fetch('http://new.sunpay.co.in/api/fund_request/', {
      method: 'POST',
      headers: {
