 ...
 //   const[circlecode, setcirclecode] = useState();
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const response = await fetch(`http://new.sunpay.co.in/api/get_providers/`);
          if (!response.ok) {
            throw new Error('Failed to fetch data');
          }
          const data = await response.json();
          
          const filteredOptions = data.filter(option => option.type === 5);
          setproviders(filteredOptions);
          
          const stateresponse = await fetch(`http://new.sunpay.co.in/api/get_state/`);
          if (!stateresponse.ok) {
            throw new Error('Failed to fetch data');
          }
    ...
    useEffect(() => { 
      const fetchuser = async () => {
          try {
            const response = await axios.get(`http://new.sunpay.co.in/api/users/${navdata.id}`)
    ...
    const handlesubmit = async (e) => {   
      ...
      // const getbill = await fetch('http://new.sunpay.co.in/api/get_biller_details/', {
      //   method: 'POST',
      //   headers: {
      ...
      // const paybill = await fetch('http://new.sunpay.co.in/api/pay_recharge/', {
      //   method: 'POST',
      //   headers: {
      //     'Content-Type': 'application/json',
      //   },
      ...
      <div>
        <div className='Rechargehistory p-4'>
          <div className='bg-slate-400 w-full p-2 border-2 border-red-200 px-6'>
            <h1 className=' font-bold text-2xl border-b-2 text-white border-gray-400'>MCD Taxes & Services</h1>
        ...
        {!show && (<div>
              ...
              <div className='mt-4 flex gap-4'>
                <label className="mt-2 text-white w-52 text-2xl">Flat No. / Consumer ID</label><br/>
                <input className=" text-lg px-4 border border-solid border-gray-300 rounded" onChange={(e) => setno(e.target.value)} type="text" placeholder="Enter ConsumerID/ Flat No" />
              </div>
