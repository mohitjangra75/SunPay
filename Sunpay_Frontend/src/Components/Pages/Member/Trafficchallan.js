    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const filteredOptions = data.filter(option => option.type === 11);
      ...
      <div>
        <div className='Rechargehistory p-4'>
          <div className='bg-slate-400 w-full p-2 border-2 border-red-200 px-6'>
            <h1 className=' font-bold text-2xl border-b-2 text-white border-gray-400'>Electricity Bill Pay</h1>
        ...
        {!show && (<div>
              ...
              <div className='mt-4 flex gap-4'>
                <label className="mt-2 text-white w-52 text-2xl">Challan No</label><br/>
                <input className=" text-lg px-4 border border-solid border-gray-300 rounded" onChange={(e) => setno(e.target.value)} type="text" placeholder="Enter Challan No" />
              </div>
